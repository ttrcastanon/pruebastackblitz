...
@Component({
  selector: 'app-Coordinacion_Pasajeros',
  templateUrl: './Coordinacion_Pasajeros.component.html',
  styleUrls: ['./Coordinacion_Pasajeros.component.scss'],
  animations: [
...
export class Coordinacion_PasajerosComponent implements OnInit, AfterViewInit {
MRaddTransportacion: boolean = false;
MRaddReservaciones: boolean = false;
MRaddInstalaciones: boolean = false;
MRaddNormal: boolean = false;
MRaddPeriodico_y_Revistas: boolean = false;
MRaddServicios: boolean = false;
  ...
  getParamsFromUrl() {
    this.route.paramMap.subscribe(
      params => {
        ...
        if (this.model.Folio) {
          this.operation = !this.Coordinacion_PasajerosForm.disabled ? "Update" : this.operation;
  ...
  rulesAfterSave() {
    ...
    if (this.operation == 'New') {
      this.brf.EvaluaQuery(" update Coordinacion_Pasajeros set Numero_de_Vuelo= " + SpartanOperationId + " where Folio=" + KeyValueInserted + "", 1, "ABC123");
    }
    //TERMINA - BRID:1889
