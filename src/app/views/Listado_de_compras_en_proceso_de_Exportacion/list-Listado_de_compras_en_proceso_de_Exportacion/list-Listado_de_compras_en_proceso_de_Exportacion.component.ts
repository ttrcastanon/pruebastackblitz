...
import { Listado_de_compras_en_proceso_de_ExportacionIndexRules } from 'src/app/shared/businessRules/Listado_de_compras_en_proceso_de_Exportacion-index-rules';
...
@Component({
  selector: "app-list-Listado_de_compras_en_proceso_de_Exportacion",
  templateUrl: "./list-Listado_de_compras_en_proceso_de_Exportacion.component.html",
  styleUrls: ["./list-Listado_de_compras_en_proceso_de_Exportacion.component.scss"],
})
export class ListListado_de_compras_en_proceso_de_ExportacionComponent extends Listado_de_compras_en_proceso_de_ExportacionIndexRules implements OnInit, AfterViewInit, AfterViewChecked {
  ...
  dataSource: Listado_de_compras_en_proceso_de_ExportacionDataSource | null;
  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;

  dataSourceTemp: Listado_de_compras_en_proceso_de_ExportacionDataSource;
  dataClipboard: any;

  constructor(
    private _Listado_de_compras_en_proceso_de_ExportacionService: Listado_de_compras_en_proceso_de_ExportacionService,
  ...
  ngOnInit() {
    ...
    this.dataSource = new Listado_de_compras_en_proceso_de_ExportacionDataSource(
      this._Listado_de_compras_en_proceso_de_ExportacionService, this._file
    );
    this.init();
    this._seguridad
...
//Fin de reglas
  ...
  remove(row: Listado_de_compras_en_proceso_de_Exportacion) {
    this._messages
      .confirmation(
      ...
      .then(() => {
        ...
        this._Listado_de_compras_en_proceso_de_ExportacionService
          .delete(+row.Folio)
          .pipe(
            catchError(() => of([])),
   ...
   */
  ActionPrint(dataRow: Listado_de_compras_en_proceso_de_Exportacion) {
...
,x.Fecha_de_Factura
    ...
    pdfMake.createPdf(pdfDefinition).download('Listado_de_compras_en_proceso_de_Exportacion.pdf');
  }
    ...
    this.dialogo
      ...
      .subscribe((optionSelected: string) => {
        ...
        if (optionSelected == '1') {
          ...
          this._Listado_de_compras_en_proceso_de_ExportacionService.listaSelAll(starRowIndex, maximumRows, condition, sort)
            .subscribe((response: any) => {
              this.dataSourceTemp = response.Listado_de_compras_en_proceso_de_Exportacions;
          ...
          this._Listado_de_compras_en_proceso_de_ExportacionService.listaSelAll(1, 9999, '', '')
            .subscribe((response: any) => {
              this.dataSourceTemp = response.Listado_de_compras_en_proceso_de_Exportacions;
              const dataToRender = this.SetTableExport(this.dataSourceTemp);
  ...
  ExportToExcel(data: any) {
    ...
    this.excelService.exportAsExcelFile(excelData, `Listado_de_compras_en_proceso_de_Exportacion  ${new Date().toLocaleString()}`);
  }
  ...
  ExportToCSV(data: any) {
    ...
    this.excelService.exportToCsv(result, 'Listado_de_compras_en_proceso_de_Exportacion',  ['Folio'    ,'Proveedor'  ,'No__Orden_de_Compra'  ,'Fecha_de_Factura' ]);
  }
...
export class Listado_de_compras_en_proceso_de_ExportacionDataSource implements DataSource<Listado_de_compras_en_proceso_de_Exportacion>
{
  private subject = new BehaviorSubject<Listado_de_compras_en_proceso_de_Exportacion[]>([]);
  ...
  constructor(private service: Listado_de_compras_en_proceso_de_ExportacionService, private _file: SpartanFileService) { }

  connect(
    collectionViewer: CollectionViewer
  ): Observable<Listado_de_compras_en_proceso_de_Exportacion[]> {
...
if (data.MRWhere.length > 0){
      ...
      .subscribe((result: any) => {
        ...
        data.columns.forEach((column, index) => {
          if (column === 'Folio') { // Clave primaria
            ...
            try {
              const longest = result.Listado_de_compras_en_proceso_de_Exportacions.map(x => (x[column]) ? x[column].toString() : '').sort((a, b) => b.length - a.length)[0];
        ...
        this.subject.next(result.Listado_de_compras_en_proceso_de_Exportacions);
  ...
  SetWhereClause(data: any): string {
    ...
    if (data.filter.Folio != "")
      condition += " and Listado_de_compras_en_proceso_de_Exportacion.Folio = " + data.filter.Folio;
    ...
    if (data.filter.Fecha_de_Factura)
      condition += " and CONVERT(VARCHAR(10), Listado_de_compras_en_proceso_de_Exportacion.Fecha_de_Factura, 102)  = '" + moment(data.filter.Fecha_de_Factura).format("YYYY.MM.DD") + "'";
  ...
  SetOrderClause(data: any): string {
    ...
    switch (data.sortField) {
      case "Folio":
        sort = " Listado_de_compras_en_proceso_de_Exportacion.Folio " + data.sortDirecction;
        break;
      case "Proveedor":
      ...
      case "Fecha_de_Factura":
        sort = " Listado_de_compras_en_proceso_de_Exportacion.Fecha_de_Factura " + data.sortDirecction;
        break;
  ...
  loadAdvanced(data: any) {
	...
	{
      if (typeof data.filterAdvanced.fromFolio != 'undefined' && data.filterAdvanced.fromFolio)
        condition += " AND Listado_de_compras_en_proceso_de_Exportacion.Folio >= " + data.filterAdvanced.fromFolio;

      if (typeof data.filterAdvanced.toFolio != 'undefined' && data.filterAdvanced.toFolio) 
        condition += " AND Listado_de_compras_en_proceso_de_Exportacion.Folio <= " + data.filterAdvanced.toFolio;
    }
    if ((typeof data.filterAdvanced.Proveedor != 'undefined' && data.filterAdvanced.Proveedor)) {
      ...
      condition += " AND Listado_de_compras_en_proceso_de_Exportacion.Proveedor In (" + Proveedords + ")";
    }
    if ((typeof data.filterAdvanced.No__Orden_de_Compra != 'undefined' && data.filterAdvanced.No__Orden_de_Compra)) {
      ...
      condition += " AND Listado_de_compras_en_proceso_de_Exportacion.No__Orden_de_Compra In (" + No__Orden_de_Comprads + ")";
    }
	...
	{
      if (typeof data.filterAdvanced.fromFecha_de_Factura != 'undefined' && data.filterAdvanced.fromFecha_de_Factura) 
        condition += " and CONVERT(VARCHAR(10), Listado_de_compras_en_proceso_de_Exportacion.Fecha_de_Factura, 102)  >= '" +  moment(data.filterAdvanced.fromFecha_de_Factura).format("YYYY.MM.DD") + "'";
      
      if (typeof data.filterAdvanced.toFecha_de_Factura != 'undefined' && data.filterAdvanced.toFecha_de_Factura) 
        condition += " and CONVERT(VARCHAR(10), Listado_de_compras_en_proceso_de_Exportacion.Fecha_de_Factura, 102)  <= '" + moment(data.filterAdvanced.toFecha_de_Factura).format("YYYY.MM.DD") + "'";
    }
