...
@Component({
  selector: 'app-dialog-generacion-de-orden-de-compras-form',
  templateUrl: './dialog-Generacion_de_Orden_de_compras-form.component.html',
  styleUrls: ['./dialog-Generacion_de_Orden_de_compras-form.component.scss'],
  animations: [
...
export class DialogGeneracionDeOrdenDeComprasFormComponent implements OnInit, AfterViewInit {
  ...
  constructor(private fb: FormBuilder,
    ...
    public dialogRef: MatDialogRef<DialogGeneracionDeOrdenDeComprasFormComponent>,
    ...
    this.Detalle_de_Gestion_de_aprobacionOC = this.data.Detalle_de_Gestion_de_aprobacionOrdenCompra
  ...
  getParamsFromUrl() {
    ...
    if (this.operation == "Update") {
      this.model.Folio = this.data?.IdGeneracionOC
  ...
  //#region Detalle_de_Generacion_de_OCItems
  ...
  async saveDetalle_de_Generacion_de_OC(Folio: number) {
    ...
    this.dataSourceDetalle_de_Generacion_de_Orden_de_Compra.data.forEach(async (d, index) => {
      ...
      if (model.Folio == null) model.Folio = 0

      if (model.Folio === 0) {
        // Add Detalle de Generación de Orden de Compra
        await this.Detalle_de_Generacion_de_OCService.insert(model).toPromise();
        ...
        if (formDetalle_de_Generacion_de_Orden_de_Compra.dirty) {
          ...
          await this.Detalle_de_Generacion_de_OCService.update(model.Folio, model).toPromise();
        }
      } else if (model.Folio > 0 && d.IsDeleted) {
  ...
  rulesOnInit() {
    ...
    if (this.operation == 'New') {
      //this.brf.SetValueFromQuery(this.Generacion_de_Orden_de_ComprasForm, "Tipo_de_Envio", this.brf.EvaluaQuery(`SELECT Tipo_de_Envio FROM Detalle_de_Agregar_Proveedor_Piezas where Seleccion_de_Proveedor = 1 and Comparativo_de_Proveedores = ${this.Detalle_de_Gestion_de_aprobacionOC[0].IdComparativoProveedores} `, 1, "ABC123"), 1, "ABC123");
      //this.brf.SetEnabledControl(this.Generacion_de_Orden_de_ComprasForm, 'Tipo_de_Envio', 0);
    }
    //TERMINA - BRID:4115
