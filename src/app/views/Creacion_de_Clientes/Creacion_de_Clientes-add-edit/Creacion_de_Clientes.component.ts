...
@Component({
  selector: 'app-Creacion_de_Clientes',
  templateUrl: './Creacion_de_Clientes.component.html',
  styleUrls: ['./Creacion_de_Clientes.component.scss'],
  animations: [
...
export class Creacion_de_ClientesComponent implements OnInit, AfterViewInit {
	...
	Creacion_de_ClientesForm: FormGroup;
	public Editor = ClassicEditor;
	model: Creacion_de_Clientes;
  ...
  constructor(private fb: FormBuilder,
    ...
    private Creacion_de_ClientesService: Creacion_de_ClientesService,
    ...
    this.model = new Creacion_de_Clientes(this.fb);
    this.Creacion_de_ClientesForm = this.model.buildFormGroup();
	
	this.Creacion_de_ClientesForm.get('Clave').disable();
    this.Creacion_de_ClientesForm.get('Clave').setValue('Auto');
    ...
    this.route
      ...
      .subscribe(v => {
        if (v.readOnly) {
          ...
          this.Creacion_de_ClientesForm.disable();
  ...
  async populateModel(id: number) {
	...
	let result =  await this.Creacion_de_ClientesService.listaSelAll(0, 1, 'Creacion_de_Clientes.Clave=' + id).toPromise();
	if (result.Creacion_de_Clientess.length > 0) {
	  
        this.model.fromObject(result.Creacion_de_Clientess[0]);

		this.Creacion_de_ClientesForm.markAllAsTouched();
		this.Creacion_de_ClientesForm.updateValueAndValidity();
  ...
  getParamsFromUrl() {
    this.route.paramMap.subscribe(
      params => {
        ...
        if (this.model.Clave) {
          this.operation = !this.Creacion_de_ClientesForm.disabled ? "Update" : this.operation;
  ...
  populateControls() {
    ...
    if (observablesArray.length > 0) {
      forkJoin(observablesArray)
        .subscribe(([ varEstatus_de_Cliente , varRespuesta ]) => {
  ...
  async saveData(): Promise<any> {
    ...
    if (this.rulesBeforeSave()) {
      const entity = this.Creacion_de_ClientesForm.value;
      entity.Clave = this.model.Clave;
	  	  
	  
	  if (this.model.Clave > 0 ) {
        await this.Creacion_de_ClientesService.update(this.model.Clave, entity).toPromise();
        ...
        await (this.Creacion_de_ClientesService.insert(entity).toPromise().then(async id => {
  ...
  async saveAndNew() {
    await this.saveData();
    if (this.model.Clave === 0 ) {
      this.Creacion_de_ClientesForm.reset();
      this.model = new Creacion_de_Clientes(this.fb);
      this.Creacion_de_ClientesForm = this.model.buildFormGroup();

    } else {
      this.router.navigate(['views/Creacion_de_Clientes/add']);
    }
  }
