...
import { Coordinacion_AvisosIndexRules } from 'src/app/shared/businessRules/Coordinacion_Avisos-index-rules';
...
@Component({
  selector: "app-list-Coordinacion_Avisos",
  templateUrl: "./list-Coordinacion_Avisos.component.html",
  styleUrls: ["./list-Coordinacion_Avisos.component.scss"],
})
export class ListCoordinacion_AvisosComponent extends Coordinacion_AvisosIndexRules implements OnInit, AfterViewInit, AfterViewChecked {
  ...
  dataSource: Coordinacion_AvisosDataSource | null;
  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;

  dataSourceTemp: Coordinacion_AvisosDataSource;
  dataClipboard: any;

  constructor(
    private _Coordinacion_AvisosService: Coordinacion_AvisosService,
  ...
  ngOnInit() {
    ...
    this.dataSource = new Coordinacion_AvisosDataSource(
      this._Coordinacion_AvisosService, this._file
    );
    this.init();
    this._seguridad
...
//Fin de reglas
  ...
  remove(row: Coordinacion_Avisos) {
    this._messages
      .confirmation(
      ...
      .then(() => {
        ...
        this._Coordinacion_AvisosService
          .delete(+row.Folio)
          .pipe(
            catchError(() => of([])),
   ...
   */
  ActionPrint(dataRow: Coordinacion_Avisos) {
...
,x.Observaciones
    ...
    pdfMake.createPdf(pdfDefinition).download('Coordinacion_Avisos.pdf');
  }
    ...
    this.dialogo
      ...
      .subscribe((optionSelected: string) => {
        ...
        if (optionSelected == '1') {
          ...
          this._Coordinacion_AvisosService.listaSelAll(starRowIndex, maximumRows, condition, sort)
            .subscribe((response: any) => {
              this.dataSourceTemp = response.Coordinacion_Avisoss;
          ...
          this._Coordinacion_AvisosService.listaSelAll(1, 9999, '', '')
            .subscribe((response: any) => {
              this.dataSourceTemp = response.Coordinacion_Avisoss;
              const dataToRender = this.SetTableExport(this.dataSourceTemp);
  ...
  ExportToExcel(data: any) {
    ...
    this.excelService.exportAsExcelFile(excelData, `Coordinacion_Avisos  ${new Date().toLocaleString()}`);
  }
  ...
  ExportToCSV(data: any) {
    ...
    this.excelService.exportToCsv(result, 'Coordinacion_Avisos',  ['Folio'    ,'Numero_de_Vuelo'  ,'Matricula'  ,'Ruta_de_Vuelo'  ,'Fecha_y_Hora_de_Salida'  ,'Calificacion'  ,'Observaciones' ]);
  }
...
export class Coordinacion_AvisosDataSource implements DataSource<Coordinacion_Avisos>
{
  private subject = new BehaviorSubject<Coordinacion_Avisos[]>([]);
  ...
  constructor(private service: Coordinacion_AvisosService, private _file: SpartanFileService) { }

  connect(
    collectionViewer: CollectionViewer
  ): Observable<Coordinacion_Avisos[]> {
...
if (data.MRWhere.length > 0){
      ...
      .subscribe((result: any) => {
        ...
        data.columns.forEach((column, index) => {
          if (column === 'Folio') { // Clave primaria
            ...
            try {
              const longest = result.Coordinacion_Avisoss.map(x => (x[column]) ? x[column].toString() : '').sort((a, b) => b.length - a.length)[0];
        ...
        this.subject.next(result.Coordinacion_Avisoss);
  ...
  SetWhereClause(data: any): string {
    ...
    if (data.filter.Folio != "")
      condition += " and Coordinacion_Avisos.Folio = " + data.filter.Folio;
    ...
    if (data.filter.Ruta_de_Vuelo != "")
      condition += " and Coordinacion_Avisos.Ruta_de_Vuelo like '%" + data.filter.Ruta_de_Vuelo + "%' ";
    if (data.filter.Fecha_y_Hora_de_Salida != "")
      condition += " and Coordinacion_Avisos.Fecha_y_Hora_de_Salida like '%" + data.filter.Fecha_y_Hora_de_Salida + "%' ";
    if (data.filter.Calificacion != "")
      condition += " and Coordinacion_Avisos.Calificacion = " + data.filter.Calificacion;
    if (data.filter.Observaciones != "")
      condition += " and Coordinacion_Avisos.Observaciones like '%" + data.filter.Observaciones + "%' ";
  ...
  SetOrderClause(data: any): string {
    ...
    switch (data.sortField) {
      case "Folio":
        sort = " Coordinacion_Avisos.Folio " + data.sortDirecction;
        break;
      ...
      case "Matricula":
        ...
        break;
      case "Ruta_de_Vuelo":
        sort = " Coordinacion_Avisos.Ruta_de_Vuelo " + data.sortDirecction;
        break;
      case "Fecha_y_Hora_de_Salida":
        sort = " Coordinacion_Avisos.Fecha_y_Hora_de_Salida " + data.sortDirecction;
        break;
      case "Calificacion":
        sort = " Coordinacion_Avisos.Calificacion " + data.sortDirecction;
        break;
      case "Observaciones":
        sort = " Coordinacion_Avisos.Observaciones " + data.sortDirecction;
        break;
  ...
  loadAdvanced(data: any) {
	...
	{
      if (typeof data.filterAdvanced.fromFolio != 'undefined' && data.filterAdvanced.fromFolio)
        condition += " AND Coordinacion_Avisos.Folio >= " + data.filterAdvanced.fromFolio;

      if (typeof data.filterAdvanced.toFolio != 'undefined' && data.filterAdvanced.toFolio) 
        condition += " AND Coordinacion_Avisos.Folio <= " + data.filterAdvanced.toFolio;
    }
    if ((typeof data.filterAdvanced.Numero_de_Vuelo != 'undefined' && data.filterAdvanced.Numero_de_Vuelo)) {
      ...
      condition += " AND Coordinacion_Avisos.Numero_de_Vuelo In (" + Numero_de_Vuelods + ")";
    }
    if ((typeof data.filterAdvanced.Matricula != 'undefined' && data.filterAdvanced.Matricula)) {
      ...
      condition += " AND Coordinacion_Avisos.Matricula In (" + Matriculads + ")";
    }
    switch (data.filterAdvanced.Ruta_de_VueloFilter) {
      case Enumerations.Enums.Filters.BeginWith:
        condition += " AND Coordinacion_Avisos.Ruta_de_Vuelo LIKE '" + data.filterAdvanced.Ruta_de_Vuelo + "%'";
        break;
      case Enumerations.Enums.Filters.Contains:
        condition += " AND Coordinacion_Avisos.Ruta_de_Vuelo LIKE '%" + data.filterAdvanced.Ruta_de_Vuelo + "%'";
        break;
      case Enumerations.Enums.Filters.EndWith:
        ...
        break;
      case Enumerations.Enums.Filters.Exact:
        condition += " AND Coordinacion_Avisos.Ruta_de_Vuelo = '" + data.filterAdvanced.Ruta_de_Vuelo + "'";
        break;
    }
    switch (data.filterAdvanced.Fecha_y_Hora_de_SalidaFilter) {
      case Enumerations.Enums.Filters.BeginWith:
        condition += " AND Coordinacion_Avisos.Fecha_y_Hora_de_Salida LIKE '" + data.filterAdvanced.Fecha_y_Hora_de_Salida + "%'";
        break;
      case Enumerations.Enums.Filters.Contains:
        condition += " AND Coordinacion_Avisos.Fecha_y_Hora_de_Salida LIKE '%" + data.filterAdvanced.Fecha_y_Hora_de_Salida + "%'";
        break;
      case Enumerations.Enums.Filters.EndWith:
        ...
        break;
      case Enumerations.Enums.Filters.Exact:
        condition += " AND Coordinacion_Avisos.Fecha_y_Hora_de_Salida = '" + data.filterAdvanced.Fecha_y_Hora_de_Salida + "'";
        break;
    }
	...
	{
      if (typeof data.filterAdvanced.fromCalificacion != 'undefined' && data.filterAdvanced.fromCalificacion)
        condition += " AND Coordinacion_Avisos.Calificacion >= " + data.filterAdvanced.fromCalificacion;

      if (typeof data.filterAdvanced.toCalificacion != 'undefined' && data.filterAdvanced.toCalificacion) 
        condition += " AND Coordinacion_Avisos.Calificacion <= " + data.filterAdvanced.toCalificacion;
    }
    switch (data.filterAdvanced.ObservacionesFilter) {
      case Enumerations.Enums.Filters.BeginWith:
        condition += " AND Coordinacion_Avisos.Observaciones LIKE '" + data.filterAdvanced.Observaciones + "%'";
        break;
      case Enumerations.Enums.Filters.Contains:
        condition += " AND Coordinacion_Avisos.Observaciones LIKE '%" + data.filterAdvanced.Observaciones + "%'";
        break;
      case Enumerations.Enums.Filters.EndWith:
        ...
        break;
      case Enumerations.Enums.Filters.Exact:
        condition += " AND Coordinacion_Avisos.Observaciones = '" + data.filterAdvanced.Observaciones + "'";
        break;
    }
