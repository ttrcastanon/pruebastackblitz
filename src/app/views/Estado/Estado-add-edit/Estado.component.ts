...
export class EstadoComponent implements OnInit, AfterViewInit {
  ...
  constructor(private fb: FormBuilder,
    ...
    this.model = new Estado(this.fb);
    this.EstadoForm = this.model.buildFormGroup();
	
	this.EstadoForm.get('Clave').disable();
    this.EstadoForm.get('Clave').setValue('Auto');
  ...
  async populateModel(id: number) {
	...
	let result =  await this.EstadoService.listaSelAll(0, 1, 'Estado.Clave=' + id).toPromise();
	if (result.Estados.length > 0) {
	  
        this.model.fromObject(result.Estados[0]);

		this.EstadoForm.markAllAsTouched();
		this.EstadoForm.updateValueAndValidity();
  ...
  getParamsFromUrl() {
    this.route.paramMap.subscribe(
      params => {
        ...
        if (this.model.Clave) {
          this.operation = !this.EstadoForm.disabled ? "Update" : this.operation;
  ...
  filterCb(combo: string, filter: string) {
    switch (combo) {
      case 'Pais': {
        this.PaisService.listaSelAll(1, 1000, filter).subscribe(x => {
          this.varPais = x.Paiss;
        });
        break;
      }
  ...
  async saveData(): Promise<any> {
    ...
    if (this.rulesBeforeSave()) {
      const entity = this.EstadoForm.value;
      entity.Clave = this.model.Clave;
	  	  
	  
	  if (this.model.Clave > 0 ) {
        await this.EstadoService.update(this.model.Clave, entity).toPromise();
        ...
        await (this.EstadoService.insert(entity).toPromise().then(async id => {
  ...
  async saveAndNew() {
    ...
    if (this.model.Clave === 0 ) {
      ...
      this.router.navigate(['views/Estado/add']);
    }
  }
