...
import { Coordinacion_HandlingIndexRules } from 'src/app/shared/businessRules/Coordinacion_Handling-index-rules';
...
@Component({
  selector: "app-list-Coordinacion_Handling",
  templateUrl: "./list-Coordinacion_Handling.component.html",
  styleUrls: ["./list-Coordinacion_Handling.component.scss"],
})
export class ListCoordinacion_HandlingComponent extends Coordinacion_HandlingIndexRules implements OnInit, AfterViewInit, AfterViewChecked {
  ...
  dataSource: Coordinacion_HandlingDataSource | null;
  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;

  dataSourceTemp: Coordinacion_HandlingDataSource;
  dataClipboard: any;

  constructor(
    private _Coordinacion_HandlingService: Coordinacion_HandlingService,
  ...
  ngOnInit() {
    ...
    this.dataSource = new Coordinacion_HandlingDataSource(
      this._Coordinacion_HandlingService, this._file
    );
    this.init();
    this._seguridad
      .permisos(AppConstants.Permisos.Coordinacion_Handling)
      .subscribe((response) => {
...
//Fin de reglas
  ...
  remove(row: Coordinacion_Handling) {
    this._messages
      .confirmation(
      ...
      .then(() => {
        ...
        this._Coordinacion_HandlingService
          .delete(+row.Folio)
          .pipe(
            catchError(() => of([])),
   ...
   */
  ActionPrint(dataRow: Coordinacion_Handling) {
...
,x.Observaciones
    ...
    pdfMake.createPdf(pdfDefinition).download('Coordinacion_Handling.pdf');
  }
    ...
    this.dialogo
      ...
      .subscribe((optionSelected: string) => {
        ...
        if (optionSelected == '1') {
          ...
          this._Coordinacion_HandlingService.listaSelAll(starRowIndex, maximumRows, condition, sort)
            .subscribe((response: any) => {
              this.dataSourceTemp = response.Coordinacion_Handlings;
          ...
          this._Coordinacion_HandlingService.listaSelAll(1, 9999, '', '')
            .subscribe((response: any) => {
              this.dataSourceTemp = response.Coordinacion_Handlings;
              const dataToRender = this.SetTableExport(this.dataSourceTemp);
  ...
  ExportToExcel(data: any) {
    ...
    this.excelService.exportAsExcelFile(excelData, `Coordinacion_Handling  ${new Date().toLocaleString()}`);
  }
  ...
  ExportToCSV(data: any) {
    ...
    this.excelService.exportToCsv(result, 'Coordinacion_Handling',  ['Folio'    ,'Numero_de_Vuelo'  ,'Matricula'  ,'Ruta_de_Vuelo'  ,'Fecha_y_Hora_de_Salida'  ,'Calificacion'  ,'Observaciones' ]);
  }
...
export class Coordinacion_HandlingDataSource implements DataSource<Coordinacion_Handling>
{
  private subject = new BehaviorSubject<Coordinacion_Handling[]>([]);
  ...
  constructor(private service: Coordinacion_HandlingService, private _file: SpartanFileService) { }

  connect(
    collectionViewer: CollectionViewer
  ): Observable<Coordinacion_Handling[]> {
...
if (data.MRWhere.length > 0){
      ...
      .subscribe((result: any) => {
        ...
        data.columns.forEach((column, index) => {
          if (column === 'Folio') { // Clave primaria
            ...
            try {
              const longest = result.Coordinacion_Handlings.map(x => (x[column]) ? x[column].toString() : '').sort((a, b) => b.length - a.length)[0];
        ...
        this.subject.next(result.Coordinacion_Handlings);
  ...
  SetWhereClause(data: any): string {
    ...
    if (data.filter.Folio != "")
      condition += " and Coordinacion_Handling.Folio = " + data.filter.Folio;
    ...
    if (data.filter.Ruta_de_Vuelo != "")
      condition += " and Coordinacion_Handling.Ruta_de_Vuelo like '%" + data.filter.Ruta_de_Vuelo + "%' ";
    if (data.filter.Fecha_y_Hora_de_Salida != "")
      condition += " and Coordinacion_Handling.Fecha_y_Hora_de_Salida like '%" + data.filter.Fecha_y_Hora_de_Salida + "%' ";
    if (data.filter.Calificacion != "")
      condition += " and Coordinacion_Handling.Calificacion = " + data.filter.Calificacion;
    if (data.filter.Observaciones != "")
      condition += " and Coordinacion_Handling.Observaciones like '%" + data.filter.Observaciones + "%' ";
  ...
  SetOrderClause(data: any): string {
    ...
    switch (data.sortField) {
      case "Folio":
        sort = " Coordinacion_Handling.Folio " + data.sortDirecction;
        break;
      ...
      case "Matricula":
        ...
        break;
      case "Ruta_de_Vuelo":
        sort = " Coordinacion_Handling.Ruta_de_Vuelo " + data.sortDirecction;
        break;
      case "Fecha_y_Hora_de_Salida":
        sort = " Coordinacion_Handling.Fecha_y_Hora_de_Salida " + data.sortDirecction;
        break;
      case "Calificacion":
        sort = " Coordinacion_Handling.Calificacion " + data.sortDirecction;
        break;
      case "Observaciones":
        sort = " Coordinacion_Handling.Observaciones " + data.sortDirecction;
        break;
  ...
  loadAdvanced(data: any) {
	...
	{
      if (typeof data.filterAdvanced.fromFolio != 'undefined' && data.filterAdvanced.fromFolio)
        condition += " AND Coordinacion_Handling.Folio >= " + data.filterAdvanced.fromFolio;

      if (typeof data.filterAdvanced.toFolio != 'undefined' && data.filterAdvanced.toFolio) 
        condition += " AND Coordinacion_Handling.Folio <= " + data.filterAdvanced.toFolio;
    }
    if ((typeof data.filterAdvanced.Numero_de_Vuelo != 'undefined' && data.filterAdvanced.Numero_de_Vuelo)) {
      ...
      condition += " AND Coordinacion_Handling.Numero_de_Vuelo In (" + Numero_de_Vuelods + ")";
    }
    if ((typeof data.filterAdvanced.Matricula != 'undefined' && data.filterAdvanced.Matricula)) {
      ...
      condition += " AND Coordinacion_Handling.Matricula In (" + Matriculads + ")";
    }
    switch (data.filterAdvanced.Ruta_de_VueloFilter) {
      case Enumerations.Enums.Filters.BeginWith:
        condition += " AND Coordinacion_Handling.Ruta_de_Vuelo LIKE '" + data.filterAdvanced.Ruta_de_Vuelo + "%'";
        break;
      case Enumerations.Enums.Filters.Contains:
        condition += " AND Coordinacion_Handling.Ruta_de_Vuelo LIKE '%" + data.filterAdvanced.Ruta_de_Vuelo + "%'";
        break;
      case Enumerations.Enums.Filters.EndWith:
        ...
        break;
      case Enumerations.Enums.Filters.Exact:
        condition += " AND Coordinacion_Handling.Ruta_de_Vuelo = '" + data.filterAdvanced.Ruta_de_Vuelo + "'";
        break;
    }
    switch (data.filterAdvanced.Fecha_y_Hora_de_SalidaFilter) {
      case Enumerations.Enums.Filters.BeginWith:
        condition += " AND Coordinacion_Handling.Fecha_y_Hora_de_Salida LIKE '" + data.filterAdvanced.Fecha_y_Hora_de_Salida + "%'";
        break;
      case Enumerations.Enums.Filters.Contains:
        condition += " AND Coordinacion_Handling.Fecha_y_Hora_de_Salida LIKE '%" + data.filterAdvanced.Fecha_y_Hora_de_Salida + "%'";
        break;
      case Enumerations.Enums.Filters.EndWith:
        ...
        break;
      case Enumerations.Enums.Filters.Exact:
        condition += " AND Coordinacion_Handling.Fecha_y_Hora_de_Salida = '" + data.filterAdvanced.Fecha_y_Hora_de_Salida + "'";
        break;
    }
	...
	{
      if (typeof data.filterAdvanced.fromCalificacion != 'undefined' && data.filterAdvanced.fromCalificacion)
        condition += " AND Coordinacion_Handling.Calificacion >= " + data.filterAdvanced.fromCalificacion;

      if (typeof data.filterAdvanced.toCalificacion != 'undefined' && data.filterAdvanced.toCalificacion) 
        condition += " AND Coordinacion_Handling.Calificacion <= " + data.filterAdvanced.toCalificacion;
    }
    switch (data.filterAdvanced.ObservacionesFilter) {
      case Enumerations.Enums.Filters.BeginWith:
        condition += " AND Coordinacion_Handling.Observaciones LIKE '" + data.filterAdvanced.Observaciones + "%'";
        break;
      case Enumerations.Enums.Filters.Contains:
        condition += " AND Coordinacion_Handling.Observaciones LIKE '%" + data.filterAdvanced.Observaciones + "%'";
        break;
      case Enumerations.Enums.Filters.EndWith:
        ...
        break;
      case Enumerations.Enums.Filters.Exact:
        condition += " AND Coordinacion_Handling.Observaciones = '" + data.filterAdvanced.Observaciones + "'";
        break;
    }
