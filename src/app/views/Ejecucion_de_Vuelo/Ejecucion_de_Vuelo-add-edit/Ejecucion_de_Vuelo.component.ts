...
@Component({
  selector: 'app-Ejecucion_de_Vuelo',
  templateUrl: './Ejecucion_de_Vuelo.component.html',
  styleUrls: ['./Ejecucion_de_Vuelo.component.scss'],
  animations: [
...
export class Ejecucion_de_VueloComponent implements OnInit, AfterViewInit {
MRaddLectura_de_Altimetros: boolean = false;
MRaddParametros: boolean = false;
MRaddPasajeros_Adicionales_MR: boolean = false;
MRaddPasajeros: boolean = false;
  ...
  ngOnInit(): void {
    ...
    this.PageTitle = "Ejecución de Vuelo";

    if (this.localStorageHelper.getItemFromLocalStorage('TituloDynamic') == 'Si') {
  ...
  getParamsFromUrl() {
    this.route.paramMap.subscribe(params => {
      ...
      if (this.model.Folio) {
        this.operation = !this.Ejecucion_de_VueloForm.disabled ? "Update" : this.operation;
  ...
  async saveAndNew() {
    await this.rulesBeforeSave();
    if (this.model.Folio === 0) {
      this.Ejecucion_de_VueloForm.reset();
      this.model = new Ejecucion_de_Vuelo(this.fb);
      this.Ejecucion_de_VueloForm = this.model.buildFormGroup();
      this.dataSourcePasajeros = new MatTableDataSource<Detalle_Ejecucion_de_vuelo_pasajeros>();
      this.PasajerosData = [];
      this.dataSourcePasajeros_Adicionales_MR = new MatTableDataSource<Detalle_Ejecucion_de_vuelo_pasajeros_adicionales>();
      this.Pasajeros_Adicionales_MRData = [];
      this.dataSourceParametros = new MatTableDataSource<Detalle_Ejecucion_Vuelo_Parametros>();
      this.ParametrosData = [];
      this.dataSourceComponentes = new MatTableDataSource<Detalle_Ejecucion_Vuelo_Componentes>();
      this.ComponentesData = [];
      this.dataSourceLectura_de_Altimetros = new MatTableDataSource<Detalle_Ejecucion_Vuelo_Altimetros>();
      this.Lectura_de_AltimetrosData = [];

    } else {
      this.router.navigate(['views/Ejecucion_de_Vuelo/add']);
    }
  }
  ...
  getNumero_de_Vuelo() {
    ...
    if (this.RoleId == 13 || this.RoleId == 9) {
      ...
      result = [{
        ...
        Description: this.localStorageHelper.getItemFromLocalStorage("Folio_Solicitud")
      }]
    }
  ...
  async onChangeNumero_de_Vuelo(option: number) {
    ...
    if (this.optionsNumero_de_Vuelo.length > 0) {
      this.optionsNumero_de_Vuelo.forEach(element => {
        if (Numero_de_Vuelo == element.Description) {
  ...
  setTimeSalidaLlegada() {
    ...
    if (this.operation) {
      ...
      if (this.operation == 'Update' || this.operation == "Consult") {
        //Asignar Hora de Llegada/Regreso en Edit
  ...
  //#region Validar Fecha y Hora:::: tipoComparacion mayor o menor que
