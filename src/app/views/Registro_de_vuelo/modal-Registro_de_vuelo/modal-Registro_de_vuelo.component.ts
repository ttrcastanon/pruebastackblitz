import { AfterViewInit, Component, ElementRef, Inject, OnInit, DoCheck, Renderer2, ViewChild, Output, EventEmitter, Input, LOCALE_ID, QueryList, ViewChildren } from '@angular/core';
...
@Component({
  selector: 'app-modal-registro-de-vuelo',
  templateUrl: './modal-Registro_de_vuelo.component.html',
  styleUrls: ['./modal-Registro_de_vuelo.component.scss'],
  providers: [
...
export class ModalRegistroDeVueloComponent implements OnInit {
  ...
  counter = 0;
  TripulacionButton: boolean = false;
  PasajerosButton: boolean = false;
  SaveTramoButton: boolean = false
  ...
  tripulanteIsUsed: boolean = true;
  pasajeroIsUsed: boolean = true;
  comandatevalue:boolean = false;
  ...
  SelectedPasajero_Detalle_Registro_Vuelo_Pasajeros: Pasajeros[] = [];
  ...
  private primeraSeleccionAeronave:boolean = true;
  ...
  // @ViewChild('paginatorTripulacion') paginatorTripulacion: MatPaginator;
  ...
  // @ViewChild('paginatorPasajeros') paginatorPasajeros: MatPaginator;
  ...
  Fecha_de_solicitud: any;
  Fecha_de_salida: any;
  numeroTramoConsulta: string = "";
  //#endregion
  ...
  constructor(private fb: FormBuilder,
    ...
    public dialogRef: MatDialogRef<ModalRegistroDeVueloComponent>,
  ...
  get CountPasajeros(){
    return this.dataSourcePasajeros.data.filter(d => !d.IsDeleted).length;
  }

  get CountTripulacion(){
    return this.dataSourceTripulacion.data.filter(d => !d.IsDeleted).length;
  }

  ngAfterViewInit(): void {
    this.dataSourceTripulacion.paginator = this.paginator.toArray()[0];
    this.dataSourcePasajeros.paginator = this.paginator.toArray()[0];
  ...
  ngOnInit(): void {
    ...
    this._seguridad.permisos(AppConstants.Permisos.Registro_de_vuelo).subscribe((response) => {
  ...
  async populateModel(id: number) {
    ...
    if (result.Registro_de_vuelos.length > 0) {
      ...
      this.numeroTramoConsulta = this.result[0].Numero_de_Tramo;
      ...
      await this.Detalle_Registro_Vuelo_TripulacionService.listaSelAll(0, 1000, 'Registro_de_vuelo.Folio=' + id).toPromise().then(fTripulacion => {
        ...
        this.dataSourceTripulacion.paginator = this.paginator.toArray()[0];
        this.dataSourceTripulacion.sort = this.sort.toArray()[0];
        this.dataSourceTripulacion._updateChangeSubscription;
        this.dataSourceTripulacion._updatePaginator;
      });
      ...
      await this.Detalle_Registro_Vuelo_PasajerosService.listaSelAll(0, 1000, 'Registro_de_vuelo.Folio=' + id).toPromise().then(fPasajeros => {
        ...
        this.dataSourcePasajeros.paginator = this.paginator.toArray()[1];
        this.dataSourcePasajeros.sort = this.sort.toArray()[1];
        this.dataSourcePasajeros._updateChangeSubscription;
        this.dataSourcePasajeros._updatePaginator;
      });
      ...
      this.Registro_de_vueloForm.get('No_Vuelo').setValue(result.Registro_de_vuelos[0].No_Vuelo, { onlySelf: false, emitEvent: true });
  ...
  addTripulacionToMR() {
    ...
    const page = Math.ceil(this.dataSourceTripulacion.data.filter(d => !d.IsDeleted).length / this.paginator.toArray()[0].pageSize);
    if (page !== this.paginator.toArray()[0].pageIndex) {
      this.paginator.toArray()[0].pageIndex = page;
    }
  }
  ...
  deleteTripulacion(element: any) {
    ...
    fgr.removeAt(index);
    let sddch = this.SelectedTripulante_Detalle_Registro_Vuelo_Tripulacion;
    sddch.splice(index, 1);

    const page = Math.ceil(index / this.paginator.toArray()[0].pageSize);
    if (page !== this.paginator.toArray()[0].pageIndex) {
      this.paginator.toArray()[0].pageIndex = page;
    }
  }
  ...
  async saveDetalle_Registro_Vuelo_Tripulacion(Folio: number) {
    this.dataSourceTripulacion.data.forEach(async (d, index) => {
      ...
      model.Vencimiento_Pasaporte = momentJS(model.Vencimiento_Pasaporte).format('MM-DD-YYYY 00:00:00 z'); 
      model.Vencimiento_Licencia = momentJS(model.Vencimiento_Licencia).format('MM-DD-YYYY 00:00:00 z'); 
      model.Vencimiento_Certificado_Medico = momentJS(model.Vencimiento_Certificado_Medico).format('MM-DD-YYYY 00:00:00 z'); 
  ...
  addFilterToControlTripulante_Detalle_Registro_Vuelo_Tripulacion(control: any, index) {
    control.valueChanges.pipe(
      ...
      switchMap((value: any) => {
        ...
        let fechaconsulta = "";
        if(Fecha_de_salida instanceof Date){
          fechaconsulta = Fecha_de_salida.toISOString();
        }
        else{
          fechaconsulta = Fecha_de_salida;
        }
        ...
        this.sqlModel.query = `select t.Clave, t.Nombre_completo from Tripulacion_Aeronave ta left join Tripulacion t on t.Clave = ta.Clave_Tripulacion ` +
          ` where t.estatus = 2 and Aeronave = '${Matricula.Description}' and t.Fecha_de_vencimiento_licencia > '${fechaconsulta}' and t.Fecha_de_vencimiento_certificado > '${fechaconsulta}'` + where;
  ...
  addPasajerosToMR() {
    ...
    const page = Math.ceil(this.dataSourcePasajeros.data.filter(d => !d.IsDeleted).length / this.paginator.toArray()[1].pageSize);
    if (page !== this.paginator.toArray()[1].pageIndex) {
      this.paginator.toArray()[1].pageIndex = page;
    }
  }
  ...
  deletePasajeros(element: any) {
    ...
    fgr.removeAt(index);
    let sddch = this.SelectedPasajero_Detalle_Registro_Vuelo_Pasajeros;
    sddch.splice(index, 1);

    const page = Math.ceil(index / this.paginator.toArray()[1].pageSize);
    if (page !== this.paginator.toArray()[1].pageIndex) {
      this.paginator.toArray()[1].pageIndex = page;
    }
  ...
  async savePasajeros(element: any) {
    ...
    if (this.PasajerosData[index].Pasajero !== formPasajeros.value.Pasajero && formPasajeros.value.Pasajero) {
      let pasajeros = await this.PasajerosService.getById(formPasajeros.value.Pasajero.Clave).toPromise();
      this.PasajerosData[index].Pasajero_Pasajeros = pasajeros;
    }

    this.PasajerosData[index].Pasajero = formPasajeros.value.Pasajero.Clave;
  ...
  editPasajeros(element: any) {
    const index = this.dataSourcePasajeros.data.indexOf(element);
    const formPasajeros = this.PasajerosItems.controls[index] as FormGroup;
    this.SelectedPasajero_Detalle_Registro_Vuelo_Pasajeros[index] = this.dataSourcePasajeros.data[index].Pasajero_Pasajeros;
    this.addFilterToControlPasajero_Detalle_Registro_Vuelo_Pasajeros(formPasajeros.controls.Pasajero, index);
  ...
  async saveDetalle_Registro_Vuelo_Pasajeros(Folio: number) {
    this.dataSourcePasajeros.data.forEach(async (d, index) => {
      ...
      if (model.Folio === 0) {
        // Add Pasajeros
        await this.Detalle_Registro_Vuelo_PasajerosService.insert(model).toPromise();
      } else if (model.Folio > 0 && !d.IsDeleted) {
        // Update pasajeros
        await this.Detalle_Registro_Vuelo_PasajerosService.update(model.Folio, model).toPromise();
      } else if (model.Folio > 0 && d.IsDeleted) {
        // delete Pasajeros
  ...
  public selectPasajero_Detalle_Registro_Vuelo_Pasajeros(event: MatAutocompleteSelectedEvent, element: any): void {
    ...
    this.SelectedPasajero_Detalle_Registro_Vuelo_Pasajeros[index] = event.option.value;
    let fgr = this.Registro_de_vueloForm.controls.Detalle_Registro_Vuelo_PasajerosItems as FormArray;
    let data = fgr.controls[index] as FormGroup;
  ...
  displayFnPasajero_Detalle_Registro_Vuelo_Pasajeros(this, element) {
    const index = this.dataSourcePasajeros.data.indexOf(element);
    return this.SelectedPasajero_Detalle_Registro_Vuelo_Pasajeros[index]?.Identificador_Alias;
  }
  ...
  _filterPasajero_Detalle_Registro_Vuelo_Pasajeros(filter: any): Observable<Pasajeros> {
    const where = filter && filter !== '' ? "Pasajeros.Identificador_Alias like '%" + filter.trim() + "%'" : '';
  ...
  addFilterToControlPasajero_Detalle_Registro_Vuelo_Pasajeros(control: any, index) {
    control.valueChanges.pipe(
      ...
      switchMap((value: any) => {
        ...
        if(typeof value == "string") return this._filterPasajero_Detalle_Registro_Vuelo_Pasajeros(value);
        return this._filterPasajero_Detalle_Registro_Vuelo_Pasajeros(value.Identificador_Alias);
      })
    ).subscribe(result => {
      ...
      this.SelectedPasajero_Detalle_Registro_Vuelo_Pasajeros[index] = this.varPasajeros.length === 0 ? new Pasajeros(this.fb) : this.varPasajeros[0];
    });
  }
  ...
  getParamsFromUrl() {
    ...
    this.primeraSeleccionAeronave = this.operation == 'New';
  ...
  populateControls() {
    ...
    if (observablesArray.length > 0) {
      forkJoin(observablesArray).subscribe(([resAeropuertos]) => {
  ...
  async saveData(): Promise<any> {
    ...
    entity.Fecha_de_salida = momentJS(entity.Fecha_de_salida).format('MM-DD-YYYY 00:00:00 z'); 
    ...
    if (this.model.Folio > 0) {
      await this.Registro_de_vueloService.update(this.model.Folio, entity).toPromise().then(async id => {
  ...
  Matricula_ExecuteBusinessRules(value) {

    console.log("Event: ", value, " Matricula: ", this.Registro_de_vueloForm.get('Matricula').value.Clave)
    let matricula = this.operation == 'Update' ? this.model.Matricula : this.Registro_de_vueloForm.get('Matricula').value.Description;
    if (matricula != undefined && value != "") {
  ...
  async rulesOnInit() {
    ...
    if (this.operation == 'New') {
      ...
      await this.spartanService.SetValueExecuteQuery(this.Registro_de_vueloForm, "Numero_de_Tramo", 'select dbo.fncGeneraTramoVuelo(' + this.localStorageHelper.getItemFromLocalStorage("Folio_Solicitud") + ')', 1, 'ABC123');
      let countNumberFligth;
      this.sqlModel.query = 'select dbo.fncGeneraTramoVuelo(' + this.localStorageHelper.getItemFromLocalStorage("Folio_Solicitud") + ')';
      this.spartanService.ExecuteQuery(this.sqlModel).subscribe(result =>{
        console.log("Tramo: ", result);
        countNumberFligth = +result
      });
      ...
      let solicitanteId = await this.brf.EvaluaQueryAsync(`SELECT Solicitante FROM Solicitud_de_Vuelo WHERE Folio = ${this.SpartanOperationId}`, 1, "ABC123")
      this.Registro_de_vueloForm.controls["Solicitante"].setValue(solicitanteId)
      ...
      if (await this.brf.EvaluaQueryAsync(`SELECT COUNT(*) from Registro_de_Vuelo where No_Vuelo = ${this.localStorageHelper.getItemFromLocalStorage("Folio_Solicitud")} `, 1, 'ABC123') == 0 || countNumberFligth == 1) {
      ...
      this.brf.SetEnabledControl(this.Registro_de_vueloForm, 'Tipo_de_vuelo', 0);
      if(this.numeroTramoConsulta == "1") {
        this.brf.SetEnabledControl(this.Registro_de_vueloForm, 'Tipo_de_vuelo', 1);
      }
    ...
    if ((+this.spartanService.SetValueExecuteQueryRT(`SELECT COUNT(*) FROM Solicitud_de_Vuelo where folio = ${this.localStorageHelper.getItemFromLocalStorage("SpartanOperationId")} and estatus = 9 `, 1, 'ABC123') == 1)
      && (this.RoleId == 9)) {
      this.brf.SetEnabledControl(this.Registro_de_vueloForm, 'Folio', 0);
      ...
      alert(" El vuelo ha sido cerrado y no se pueden realizar modificaciones, favor de revisar");
      this.SaveTramoButton = true;
      this.PasajerosButton = true;
      this.TripulacionButton = true;
    }
  ...
  rulesAfterSave() {
    ...
    let Numero_de_Tramo = this.Registro_de_vueloForm.controls['Numero_de_Tramo'].value;
    let tipoVuelo =this.Registro_de_vueloForm.controls['Tipo_de_vuelo'].value;
    ...
    if (this.operation == 'New') {
      ...
      this.sqlModel.query = `SELECT Numero_de_Vuelo FROM Solicitud_de_Vuelo where Folio = ${Folio_Solicitud}`
    ...
    if (this.operation == 'Update') {      
      ...
      this.sqlModel.query = ` EXEC uspActualizaCantidadPasajerosTramo ${Folio_Registro}`;
      this.spartanService.ExecuteQuery(this.sqlModel).subscribe({
        next: (response) => {
          console.log("actualizado pasajeros: ", response);
        }
      });
    }
    //TERMINA - BRID:2057
    ...
    if (this.operation == 'New' || this.operation == 'Update') {
      ...
      let query = `EXEC sp_UpdTipoDeVuelo ${Folio_Solicitud}, ${tipoVuelo}, '${Numero_de_Tramo}'`;
  ...
  rulesBeforeSave(boton: string = null) {
    ...
    //BRID:5545 - Validacion de fechas, no puede ser menor a la fecha del tramo anterior - Autor: Lizeth Villa - Actualización: 8/30/2021 11:40:26 AM
    if (this.brf.EvaluaQuery(`EXEC spValidacionFechaAnterior ${this.Folio_Solicitud}, ${Numero_de_Tramo}, '${formatFecha_de_salida}' `, 1, 'ABC123') == this.brf.TryParseInt('1', '1')) {
      this.brf.ShowMessage("La fecha de salida no puede ser menor a la fecha de salida del tramo anterior.");

      result = false;
    }

    if (this.dataSourceTripulacion.data.length == 0) {
      alert("Falta agregar la tripulación al tramo.");
  ...
  setFecha_de_Solicitud() {
    ...
    this.brf.SetCurrentDateToField(this.Registro_de_vueloForm, "Fecha_de_solicitud");
  }
  //#endregion
  ...
  setFecha_de_salida(option) {
    ...
    //this.brf.SetCurrentDateToField(this.Registro_de_vueloForm, "Fecha_de_salida");
  }
  //#endregion
  ...
  filterTripulante(index: any) {
    ...
    this.sqlModel.query = `select t.Clave, t.Nombre_completo from Tripulacion_Aeronave ta left join Tripulacion t on t.Clave = ta.Clave_Tripulacion ` +
      ` where t.estatus = 2 and Aeronave = '${Matricula.Description}' and t.Fecha_de_vencimiento_licencia > '${fechaconsulta}'  and t.Fecha_de_vencimiento_certificado > '${fechaconsulta}'`;

    this.spartanService.ExecuteQueryDictionary(this.sqlModel).subscribe({
      next: (data) => {
        let response = this.setResponseTripulante(data)
        this.varTripulacion = response
        this.comandatevalue = true;
      },
  ...
  setPasaportes(index) {
    ...
    this.spartanService.ExecuteQuery(this.sqlModel).subscribe({
      next: (response) => {
        ...
        this.brf.applyConditionsToControl(formTripulacion, 'Vencimiento_Pasaporte', false, true, true);
      }
    })
