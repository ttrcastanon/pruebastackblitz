...
@Component({
  ...
  templateUrl: './Reabrir_vuelo.component.html',
  styleUrls: ['./Reabrir_vuelo.component.scss'],
  animations: [
...
export class Reabrir_vueloComponent implements OnInit, AfterViewInit {
  ...
  getParamsFromUrl() {
    this.route.paramMap.subscribe(
      params => {
        ...
        if (this.model.Folio) {
          this.operation = !this.Reabrir_vueloForm.disabled ? "Update" : this.operation;
...
displayFnNumero_de_Vuelo(option: Solicitud_de_Vuelo) {
  ...
  async save() {
    await this.saveData();
    if(this.localStorageHelper.getItemFromLocalStorage("Reabrir_vueloWindowsFloat") == "1"){
      this.localStorageHelper.setItemToLocalStorage("Reabrir_vueloWindowsFloat", "0");
...
if(  this.operation == 'New' ) {
  this.brf.EvaluaQuery("UPDATE solicitud_de_vuelo SET Vuelo_Reabierto = 1 where folio = "+ this.localStorageHelper.getItemFromLocalStorage('SpartanOperationId')+"", 1, "ABC123"); 
  this.brf.EvaluaQuery("UPDATE solicitud_de_vuelo SET Estatus = 6 where folio = "+ this.localStorageHelper.getItemFromLocalStorage('SpartanOperationId')+" ", 1, "ABC123");
} 
//TERMINA - BRID:5669
...
if(  this.operation == 'New' ) {
  this.brf.EvaluaQuery(" exec uspGeneraReaperturaVuelo "+ this.localStorageHelper.getItemFromLocalStorage('SpartanOperationId')+"", 1, "ABC123"); 
  this.brf.EvaluaQuery(" EXEC Cancela_Prefacturas_Reapertura_Vuelo "+ this.localStorageHelper.getItemFromLocalStorage('SpartanOperationId')+"", 1, "ABC123");
} 
//TERMINA - BRID:5763
