...
export class CiudadComponent implements OnInit, AfterViewInit {
  ...
  constructor(private fb: FormBuilder,
    ...
    this.model = new Ciudad(this.fb);
    this.CiudadForm = this.model.buildFormGroup();
	
	this.CiudadForm.get('Clave').disable();
    this.CiudadForm.get('Clave').setValue('Auto');
  ...
  async populateModel(id: number) {
	...
	let result =  await this.CiudadService.listaSelAll(0, 1, 'Ciudad.Clave=' + id).toPromise();
	if (result.Ciudads.length > 0) {
	  
        this.model.fromObject(result.Ciudads[0]);
  ...
  getParamsFromUrl() {
    this.route.paramMap.subscribe(
      params => {
        ...
        if (this.model.Clave) {
          this.operation = !this.CiudadForm.disabled ? "Update" : this.operation;
  ...
  filterCb(combo: string, filter: string) {
    switch (combo) {
      case 'Estado': {
        this.EstadoService.listaSelAll(1, 1000, filter).subscribe(x => {
          this.varEstado = x.Estados;
        });
        break;
      }
  ...
  async saveData(): Promise<any> {
    ...
    if (this.rulesBeforeSave()) {
      const entity = this.CiudadForm.value;
      entity.Clave = this.model.Clave;
	  	  
	  
	  if (this.model.Clave > 0 ) {
        await this.CiudadService.update(this.model.Clave, entity).toPromise();
        ...
        await (this.CiudadService.insert(entity).toPromise().then(async id => {
  ...
  async saveAndNew() {
    ...
    if (this.model.Clave === 0 ) {
      ...
      this.router.navigate(['views/Ciudad/add']);
    }
  }
